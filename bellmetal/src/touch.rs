use crate::{
    Bell, Change, ChangeAccumulator, FullProvingContext, Method, MusicScoring, NaiveProver,
    PlaceNotation, ProvingContext, Stage, Stroke, TouchIterator, Transposition,
};
...
static FALSENESS_COLOURS: [&str; 14] = [
    "91;1", "92;1", "93;1", "94;1", "95;1", "96;1", "97;1", "31", "32", "33", "34", "35", "36",
    "37",
];
...
impl Row<'_> {
    ...
    fn write_annotated_string<T: MusicScoring>(
        &self,
        string: &mut String,
...
impl Touch {
    ...
    pub fn fragment_bell_iterator<'a>(
        &'a self,
        start: usize,
        end: usize,
    ) -> Box<dyn Iterator<Item = &'a Bell> + 'a> {
        let stage = self.stage.as_usize();

        if end == self.length + 1 {
            Box::new(
                self.bells[start * stage..]
                    .iter()
                    .chain(self.leftover_change.slice().iter()),
            )
        } else {
    ...
    pub fn append_iterator<'b>(&mut self, iterator: &impl TouchIterator<'b>) {
        ...
        self.ruleoffs
            .extend(iterator.ruleoff_iter().map(|x| x + len));
        self.calls
            .extend(iterator.call_iter().map(|(ind, call)| (ind + len, call)));
        self.method_names.extend(
            iterator
                .method_name_iter()
                .map(|(ind, name)| (ind + len, name.to_string())),
        );

        self.leftover_change
            .overwrite_from_iterator(&mut iterator.leftover_change_iter());

        self.length += iterator.length();
    }

    pub fn extend_with_place_notation<'a>(
        &mut self,
        pns: impl IntoIterator<Item = &'a PlaceNotation>,
    ) {
        ...
        for p in pns {
            ...
            self.leftover_change
                .multiply_iterator_into(p.iter(), &mut temp_change);
    ...
    pub fn pretty_string_multi_column<T: MusicScoring>(
        &self,
        columns: usize,
        truth: Option<&Vec<Vec<usize>>>,
    ) -> String {
        let truth_table = match truth {
            Some(t) => falseness_to_table(t),
            None => self.full_truth_table(),
        };
        ...
        let mut fragments: HashMap<(usize, usize), String> =
            HashMap::with_capacity(self.length * 2 + columns);
        ...
        let mut final_string = (0..height)
            .map(|y| {
                (0..=width)
                    .map(|x| fragments.get(&(x, y)).unwrap_or(&blank_string))
    ...
    pub fn overwrite_from_string(&mut self, string: &str) {
        let (stage, length) = {
            ...
            match potential_stage {
                ...
                None => {
                    panic!("Cannot create an empty touch");
                }
            }
        };
...
impl Touch {
    ...
    pub fn from_changes(changes: &[Change], leftover_change: Change) -> Touch {
        let mut bells: Vec<Bell> =
            Vec::with_capacity(changes.len() * leftover_change.stage().as_usize());
    ...
    pub fn from_iterator<'b, I>(iterator: &I) -> Touch
    where
        I: TouchIterator<'b>,
        I: Sized,
    {
...
impl<'a> TouchIterator<'a> for BasicTouchIterator<'a> {
    ...
    type MethodNameIter =
        MethodNameDerefIter<'a, std::collections::hash_map::Iter<'a, usize, String>>;
...
mod tests {
    use crate::{
        canon_full_cyclic, one_part_spliced_touch, Call, DefaultScoring, Method, PlaceNotation,
        Stage, Touch, Transposition,
    };
    ...
    fn pretty_string() {
        ...
        let plain_bob = Method::from_str("Plain Bob Major", "-18-18-18-18,12", Stage::MAJOR);
...
\x1b[94m32\x1b[0m changes, \x1b[92mtrue\x1b[0m.  Score: \x1b[93m50\x1b[0m. 8 4-bell runs (4f, 4b)"
        ...
        for s_ref in &TOUCH_STRINGS {
            ...
            for row in touch.row_iterator() {
                for b in row.slice() {
                    match chars.next() {
                        Some(c) => {
                            assert_eq!(b.as_char(), c);
                        }
                        None => {
                            panic!("Touch yielded too many bells");
                        }
                    }
                }
    ...
    fn inversion() {
        for (pn, stage) in &[
            ...
            (
                "36x7T.18x9T.50.36.14x1470.5T.16x9T.30.18x14.3T.50.14x1T,1T",
                Stage::MAXIMUS,
            ),
        ] {
