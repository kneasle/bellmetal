...
use crate::{Bell, Change, ChangeCollectIter, Place, Stage, Touch, TouchIterator, Transposition};
...
impl ProvingContext for NaiveProver {
    ...
    fn prove<'a>(&mut self, iter: &impl TouchIterator<'a>) -> bool {
        let mut changes: Vec<Change> =
            ChangeCollectIter::new(iter.bell_iter(), iter.stage()).collect();
    ...
    for CompactHashIterator<'_, I, T>
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        while fill_from_iterator(self.bell_iter, &mut self.hash_prover.temporary_slice) {
            (self.canon_func)(
                &self.hash_prover.temporary_slice,
                &mut self.hash_prover.temporary_change,
            );
    ...
    if slice[0] == Bell::from(0) {
        ...
        for (i, val) in slice.iter().enumerate().skip(1) {
            ...
            if new_bell <= 0 {
                change.set_bell(
                    Place::from(i),
                    Bell::from((stage as isize - 1 + new_bell) as usize),
                );
            } else {
...
mod tests {
    ...
    fn compact_hash() {
        ...
        for (t, b) in full_proof_test_touches() {
            assert_eq!(
                CompactHashProver::from_stage(t.stage).full_prove_touch(&t),
                b
            );
        }
    }
    ...
    fn canonical_proving() {
        for (touch, truth) in &[
            ...
            assert_eq!(
                HashProver::from_stage(t.stage)
                    .prove_touch_canonical(&t, canon_fixed_treble_cyclic),
                *truth
            );
            assert_eq!(
                HashProver::from_stage(t.stage)
                    .prove_canonical(&t.iter(), canon_fixed_treble_cyclic),
                *truth
            );
            assert_eq!(
                CompactHashProver::from_stage(t.stage)
                    .prove_canonical(&t.iter(), canon_fixed_treble_cyclic),
                *truth
            );
            assert_eq!(
    ...
    fn full_canonical_proving() {
        for (touch, truth) in &[
            ...
            let mut naive_truth_touch =
                NaiveProver {}.full_prove_touch_canonical(&t, canon_fixed_treble_cyclic);
            let mut naive_truth =
                NaiveProver {}.full_prove_canonical(&t.iter(), canon_fixed_treble_cyclic);
