use crate::{Call, Change, Method, Touch, TouchIterator, Transposition};

use itertools::Itertools;
use std::collections::{HashMap, HashSet};
use std::iter::repeat;

pub fn single_method_touch(method: &Method, mnemonic: &str, calls: &[Vec<&Call>]) -> Touch {
...
pub fn one_part_spliced_touch_from_indices<'a>(
    methods: impl Iterator<Item = (&'a str, &'a Method)> + Clone,
    calls: &[Vec<&Call>],
) -> Touch {
    // Find the stage and length of the touch (and make sure that methods is non-empty)
    let mut method_iter = methods.clone();

    let first_method = method_iter
        .next()
        .expect("Can't have a touch with no methods.")
        .1;
    ...
    macro_rules! process_lead {
        ($method : expr, $calls : expr) => {
            touch.append_iterator(
                &$method
                    .get_lead_fragment_with_calls(
                        0,
                        $method.lead_length(),
                        $calls.iter().map(|x| *x),
                    )
                    .iter()
                    .transfigure(&current_lead_head),
            );
...
mod tests {
    use crate::{
        one_part_spliced_touch, Call, Change, DefaultScoring, Method, Stage, TouchIterator,
    };
    ...
    fn one_part_spliced() {
        ...
        for (input_string, summary) in &[
            (
                "CoCa",
                "64 changes, true.  Score: 40. 7 4-bell runs (2f, 5b)",
            ),
            (
                "   CoXXLDKJFLCa    ",
                "64 changes, true.  Score: 40. 7 4-bell runs (2f, 5b)",
            ...
            (
                "B-P - \0Co Ca\t\n-B** X-E-B-",
                "208 changes, true.  Score: 71. 23 4-bell runs (11f, 12b)",
            ),
            (
                "BBCa",
                "96 changes, false.  Score: 58. 11 4-bell runs (4f, 7b)",
            ),
        ] {
