...
impl<T: Iterator> Iterator for AndNext<T>
where
    T::Item: Copy,
{
...
static EXTENTS: [&[u8]; CACHED_EXTENTS] = [
    ...
    &[
        3, 2, 0, 1, 2, 3, 0, 1, 2, 0, 3, 1, 2, 0, 1, 3, 3, 0, 2, 1, 0, 3, 2, 1, 0, 2, 3, 1, 0, 2,
        1, 3, 3, 0, 1, 2, 0, 3, 1, 2, 0, 1, 3, 2, 0, 1, 2, 3, 3, 2, 1, 0, 2, 3, 1, 0, 2, 1, 3, 0,
        2, 1, 0, 3, 3, 1, 2, 0, 1, 3, 2, 0, 1, 2, 3, 0, 1, 2, 0, 3, 3, 1, 0, 2, 1, 3, 0, 2, 1, 0,
        3, 2, 1, 0, 2, 3,
    ],
];
...
mod tests {
    ...
    use crate::{closure, extent, Change, Stage};
    ...
    fn change_closure() {
        ...
        assert_eq!(
            ...
            vec![Change::from("12345678"), Change::from("87654321")]
        );

        assert_eq!(closure(&Change::from("1")), vec![Change::from("1"),]);

        assert_eq!(
            closure(&Change::from("123456789")),
            vec![Change::from("123456789"),]
        );
        ...
        assert_eq!(closure(&Change::from("")), vec![Change::from(""),]);
    }
